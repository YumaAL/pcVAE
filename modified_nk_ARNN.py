...
def _normalize(log_psi: Array, machine_pow: int) -> Array: #need to modify to not norm the :latent_dim output
    """
    Normalizes log_psi to have L2-norm 1 along the last axis.
    """
    latent_dim = 5
    return log_psi.at[:,latent_dim:,:].set(log_psi[:,latent_dim:,:] - 1 / machine_pow * jax.scipy.special.logsumexp(
        machine_pow * log_psi[:,latent_dim:,:].real, axis=-1, keepdims=True
    ))
